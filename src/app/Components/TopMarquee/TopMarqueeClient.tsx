...
const TopMarqueeClient: React.FC<Props> = ({ announcement }) => {
  const [repeatCount, setRepeatCount] = useState<number>(2);
  const marqueeText = announcement || "Welcome to Delfyle";
  const containerRef = useRef<HTMLDivElement>(null);
  const textRef = useRef<HTMLSpanElement>(null);

  // Calculate repeat count dynamically
  useEffect(() => {
    const calculateRepeatCount = () => {
      const containerWidth = containerRef.current?.offsetWidth || 0;
      const textWidth = textRef.current?.offsetWidth || 1;
      const minWidthToFill = containerWidth * 2;
      const count = Math.ceil(minWidthToFill / textWidth);
      setRepeatCount(Math.max(2, count));
    };
...
export default TopMarqueeClient;
