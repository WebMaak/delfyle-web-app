...
<td data-label="Action" style={{display:'flex'}}>
  ...
  <span className={styles.scrollText}>
    Scroll right using SHIFT + Mouse Wheel or Trackpad
      ...
      <div className={styles.pagination}>
          <button className={styles.paginationBtn} onClick={() => setBackofficePage(p => Math.max(1, p - 1))} disabled={backofficePage === 1}>← Previous</button>
        <div className={styles.pageNumbers}>
            <span>Page {backofficePage} of {totalBackofficePages}</span>
        </div>
          <button className={styles.paginationBtn} onClick={() => setBackofficePage(p => Math.min(totalBackofficePages, p + 1))} disabled={backofficePage === totalBackofficePages}>Next →</button>
      </div>
    </div>
  );
  };
